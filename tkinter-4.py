...
# * 사이즈는 어디에나 사용할 수 있을 것 같은 생각이 들어 클래스로 지정해보기로함
# * getter의 형태가 자바스크립트 클래스와 다른점을 발견함
...
# ? geometry_value 변수 : 위에서 선언한 클래스를 할당하면서 필요한 값을 가져오기위해 getter인 size를 불러옴
# * 값을 불러오다보니 자바스크립트는 타입을 지정할 수 있었는데, 파이썬에서는 무조건 튜플로 처리되는 것을 확인 할 수 있었음
# * 코드양으로 보았을땐 더 길어진 현상이 일어났지만, 사이즈를 사용하는 일이 많아지면 조금 편해질 듯
...
# ? Button() 첫글자가 대문자인 것으로 보아 class문법으로 제작된 것을 유추 할 수 있었다.
